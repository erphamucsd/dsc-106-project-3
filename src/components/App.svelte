...
<main>
  ...
  <h2>Write-Up:</h2>
  <div class="writeup1">
    <p> For our project, we decided to make a visualization depicting the change in the sea surface temperature over the months. Since we’re tracking the change in temperature over time, we utilized a line chart to create our graph. We encoded a line for every year, starting from 1979 to 2024. To differentiate the lines, we made each year represent a different color, with the years corresponding to high temperatures in red and the cooler temperatures in blue. Lastly, we scaled the y-axis to start at 19.6 degrees rather than 0 degrees because all of the temperatures are above 19.6 degrees. By doing this, we can zoom in on the visualization and have a better view of the trends.
    </p>
